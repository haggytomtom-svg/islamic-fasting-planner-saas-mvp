...
const isEmailConfigured = apiKey && !apiKey.includes("placeholder");
...
export async function sendVerificationEmail(email: string, token: string) {
  if (!resend) {
    console.log(`[Email Mock] Verification link for ${email}: ${getAppUrl()}/api/auth/verify-email?token=${token}`);
    return;
  }
  try {
...
export async function sendMfaOtpEmail(email: string, otp: string) {
  if (!resend) {
    console.log(`[Email Mock] MFA OTP for ${email}: ${otp}`);
    return;
  }
  try {
    await resend.emails.send({
